...
### 7. Mobile Testing (Ngrok) - Optional

Gunakan ngrok untuk menguji fitur kamera scanner di HP atau share project ke luar.

1.  Jalankan ngrok (di terminal baru):
    ```bash
    ngrok http 8000
    ```
2.  Dapatkan URL publik (contoh: `https://xxx.ngrok.io`).
3.  Update `.env`:
    ```ini
    APP_URL=https://xxx.ngrok.io
    ```
4.  **Penting**: Jalankan `npm run build` sebelum diakses via HP agar CSS/JS termuat sempurna tanpa tergantung Vite dev server di localhost.
5.  Buka URL ngrok di browser HP. HTTPS wajib aktif agar kamera scanner bisa jalan.
...
1.  Pastikan phpMyAdmin sudah terinstall di Laragon (jika belum, download dari _Menu > Tools > Quick add > phpMyAdmin_).
...
6.  Buka terminal di folder project (`C:\laragon\www\ARTIKA`), lalu jalankan migrasi ulang:
    ...
    php artisan migrate:fresh --seed
    ```
    _(Ini akan membuat ulang tabel & data default. Data transaksi lama akan hilang kecuali Anda punya backup SQL)._
7.  Akses web kembali.
...
Mirip dengan Quick Start, namun perhatikan _permission_ folder.
...
- Jika diakses dari luar localhost (Public/Mobile/Ngrok), jalankan `npm run build` sekali saja, lalu matikan `npm run dev`. Aplikasi akan menggunakan file static hasil build.
