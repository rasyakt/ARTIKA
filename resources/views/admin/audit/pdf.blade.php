...
<head>
    ...
    <title>{{ __('admin.audit_report') }}</title>
    <style>
        body {
...
<body>
    ...
    <div class="header">
        ...
        <p class="report-title">{{ __('admin.system_audit_report') }}</p>
        <p style="margin: 5px 0; font-size: 12px;">{{ __('common.period') }}:
            {{ \Carbon\Carbon::parse($startDate)->format('d M Y') }} s/d
            {{ \Carbon\Carbon::parse($endDate)->format('d M Y') }}</p>
        <p style="margin: 5px 0; font-size: 11px; color: #666;">{{ __('common.printed_at') }}:
            {{ now()->format('d M Y H:i:s') }}</p>
    </div>

    <!-- Summary Statistics -->
    <div class="summary-section">
        <h3>📊 {{ __('admin.report_summary') }}</h3>
        <div class="stats-grid">
            <div class="stat-box">
                <div class="stat-label">{{ __('admin.total_logs') }}</div>
            ...
            <div class="stat-box">
                <div class="stat-label">{{ __('admin.total_transaction_value') }}</div>
                <div class="stat-value">Rp{{ number_format($summary['total_amount'], 0, ',', '.') }}</div>
            </div>
            <div class="stat-box">
                <div class="stat-label">{{ __('admin.avg_transaction') }}</div>
                <div class="stat-value">
                    Rp{{ $summary['total_transactions'] > 0 ? number_format($summary['total_amount'] / $summary['total_transactions'], 0, ',', '.') : 0 }}
                </div>
            </div>
        </div>
    </div>
    ...
    @if($summary['by_payment_method']->count() > 0)
        <div class="summary-section">
            <h3>💳 {{ __('admin.payment_method_summary') }}</h3>
            <table class="summary-table">
                <thead>
                    <tr>
                        <th>{{ __('admin.payment_method') }}</th>
                        <th class="text-center">{{ __('admin.number_of_transactions') }}</th>
                        <th class="amount">{{ __('admin.total_value') }}</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($summary['by_payment_method'] as $method => $data)
                        <tr>
                            <td>{{ $method ?? __('common.unknown') }}</td>
    ...
    @if($summary['by_user']->count() > 0)
        <div class="summary-section">
            <h3>👤 {{ __('admin.activity_by_user') }}</h3>
            <table class="summary-table">
                <thead>
                    <tr>
                        <th>{{ __('common.name') }}</th>
                        <th class="text-center">{{ __('admin.total_activities') }}</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($summary['by_user'] as $userId => $data)
                        <tr>
                            <td>{{ $data['user']?->name ?? __('common.unknown') }}</td>
                            <td class="text-center">{{ $data['count'] }}</td>
                        </tr>
                    @endforeach
                </tbody>
    ...
    @if($summary['by_action']->count() > 0)
        <div class="summary-section">
            <h3>📋 {{ __('admin.activity_by_type') }}</h3>
            <table class="summary-table">
                <thead>
                    <tr>
    ...
    <div class="details-section">
        <h3>📝 {{ __('admin.detailed_activity_logs') }}</h3>
        <table class="details-table">
            <thead>
                <tr>
                    ...
                    <th>{{ __('admin.activity_type') }}</th>
                    <th>{{ __('admin.model_reference') }}</th>
            ...
            <tbody>
                ...
                @empty
                    <tr>
                        <td colspan="7" class="text-center">{{ __('admin.no_activity_data') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="footer">
        ...
        <p>{{ __('admin.footer_internal_audit') }}</p>
